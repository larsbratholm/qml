...
estimator = wrappers.OAMNN(iterations=400, batch_size=70, scoring_function="mae", tensorboard=False, l2_reg=0.03906198710720926,
...
filenames = glob.glob("/Volumes/Transcend/repositories/Aglaia/data/qm7/*.xyz")[:1000]
# filenames = glob.glob("/Volumes/Transcend/data_sets/vr_ccsd/*.xyz")[:10]
# filenames = glob.glob("/Volumes/Transcend/data_sets/OH_squalane_model/geoms/training/*.xyz")[:1000]
filenames.sort()
energies = np.loadtxt('/Volumes/Transcend/repositories/Aglaia/data/qm7/hof_qm7.txt', usecols=[1])[:1000]
# energies = np.loadtxt('/Volumes/Transcend/data_sets/vr_ccsd/properties.txt', usecols=[1])[:10]
