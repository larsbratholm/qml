...
def tensormol_acsf(xyzs, Zs, elements, element_pairs, radial_cutoff=10.0, angular_cutoff=10.0,
    ...
    xyzs = tf.constant(xyzs, dtype=tf.float32)
    ...
    atom_Z_indices = tf.cast(
        tf.where(tf.equal(tf.expand_dims(atom_Zs, axis=1), tf.expand_dims(elements, axis=0)))[:, 1], tf.int32)
